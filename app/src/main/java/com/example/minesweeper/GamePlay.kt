...
class GamePlay: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(flag==1){
            ...
            var row = intent.getIntExtra("RowNumber",0)
            var col = intent.getIntExtra("ColumnNumber",0)
            var mine = intent.getIntExtra("NumebrOfMine",0)
...
//        if user lost then sending the result to result activity
        if(status == Status.WON){
            ...
            val intent= Intent(this, Result_of_the_game::class.java).apply{
